...
namespace RadioReport.PelvisMRT.Repo.Repositories
{
    ...
    {
        public LymphNodesPageRepository(PelvisMRTDbContext dbContext, IMinIOWrapper minIOWrapper, ILogger<LymphNodesPageRepository> logger)
            : base(dbContext, minIOWrapper, nameof(PelvisMRT), logger)
        {
        }

        protected override Expression<Func<Guid, LymphNodesFinding, bool>> HasImageFile => (Guid imageFileId, LymphNodesFinding f) => f.ImageFileId == imageFileId;
        ...
        public override async Task Delete(Guid id)
        {
            ...
            if (toRemove != null)
            {
                var toRemoveFromFindings = toRemove.Findings?.ToList().FindAll(p => p.ImageFileId.HasValue) ?? Enumerable.Empty<LymphNodesFinding>();
