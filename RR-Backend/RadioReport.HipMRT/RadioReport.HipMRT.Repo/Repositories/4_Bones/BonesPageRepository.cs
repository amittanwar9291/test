...
namespace RadioReport.HipMRT.Repo.Repositories
{
    ...
    {
        public BonesPageRepository(HipMRTDbContext dbContext, IMinIOWrapper minIOWrapper, ILogger<BonesPageRepository> logger)
            : base(dbContext, minIOWrapper, nameof(HipMRT), logger)
        {
        }
        ...
        public override async Task<BonesPage> GetByIdAsync(Guid id, string[] includes, Expression<Func<BonesPage, bool>> predicate = null) => await GetByIdAsync(id, includes, false);
