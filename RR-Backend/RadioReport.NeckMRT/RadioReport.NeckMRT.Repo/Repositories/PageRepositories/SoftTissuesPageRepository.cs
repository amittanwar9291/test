...
namespace RadioReport.NeckMRT.Repo.Repositories
{
    ...
    {
        public SoftTissuesPageRepository(NeckMRTDbContext dbContext,  IMinIOWrapper minIOWrapper, ILogger<SoftTissuesPageRepository> logger) 
            : base(dbContext, minIOWrapper, nameof(NeckMRT), logger)
        {
        }
        
        protected override Expression<Func<Guid, SoftTissuesFinding, bool>> HasImageFile => (Guid imageFileId, SoftTissuesFinding f) =>
            f.ImageFileId == imageFileId;
        
        public override async Task Delete(Guid id)
        {
            var imageFilePropertyPath = nameof(SoftTissuesPage.Findings) + "." +
                                        nameof(SoftTissuesFinding.UploadedImageFile);
            ...
            if (toRemove != null)
            {
                var toRemoveFromFindings = toRemove.Findings?.ToList().FindAll(p => p.ImageFileId.HasValue) ??
                                           Enumerable.Empty<SoftTissuesFinding>();
        ...
        public override async Task<SoftTissuesPage> GetByIdAsync(Guid id, string[] includes, Expression<Func<SoftTissuesPage, bool>> predicate = null) =>
