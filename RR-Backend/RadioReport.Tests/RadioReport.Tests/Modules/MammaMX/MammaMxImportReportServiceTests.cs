...
using RadioReport.MammaMX.Domain.Services;
using Xunit;

namespace RadioReport.Tests.Modules.MammaMX
{
    public class MammaMxImportReportServiceTests
    {
        private readonly MammaMXImportReportService _classUnderTest;
        ...
        public MammaMxImportReportServiceTests()
        {
            var fakeLogger = A.Fake<ILogger<MammaMXImportReportService>>();
            ...
            _classUnderTest = new MammaMXImportReportService(fakeLogger, _kafkaProducerFake, _moduleContextFake, _dicomValueParserFake, _httpAuthContextFake,
        ...
        public async void ImportReport_ValuesImportable_ValuesImportedProperly()
        {
            ...
            {
                new PageInfo { TypeName = PageTypeNames.MammaMxPatientIndication },
                new PageInfo { TypeName = PageTypeNames.MammaMxTechnology }
            };
            ...
            A.CallTo(() => _moduleContextFake.LocalizationPageName).Returns(PageTypeNames.MammaMxPatientIndication);
            A.CallTo(() => _moduleContextFake.TechnologyPageName).Returns(PageTypeNames.MammaMxTechnology);
            A.CallTo(() => _moduleContextFake.TotalPageCount).Returns((byte) 2);
            A.CallTo(() => _moduleContextFake.Type).Returns(ModuleType.MammaMX);
            ...
            A.CallTo(() => _dicomValueParserFake.ParseAgeString(A<StringImportValue>.That.Matches(v => v.RadioReportId == "uni_01_009")))
                .Returns(expectedAge);
            A.CallTo(() => _patientIndicationPageManagerFake.AddAsync(A<PatientIndicationPage>.Ignored, A<bool>.Ignored)).Returns(
                new PatientIndicationPage
                {
            ...
            Assert.Equal(ModuleType.MammaMX.GetReportTypeName(), result.ExaminationType);
