...
using RadioReport.MammaMRT.Domain.Services;
using Xunit;

namespace RadioReport.Tests.Modules.MammaMRT
{
    public class MammaMrtImportReportServiceTests
    {
        private readonly MammaMRTImportReportService _classUnderTest;
        ...
        private readonly IPageManager<PatientIndicationPage> _patientIndicationPageManagerFake;
        private readonly IPageManager<TechnologyPage> _technologyPageManagerFake;

        public MammaMrtImportReportServiceTests()
        {
            var fakeLogger = A.Fake<ILogger<MammaMRTImportReportService>>();
            ...
            _patientIndicationPageManagerFake = A.Fake<IPageManager<PatientIndicationPage>>();
            _technologyPageManagerFake = A.Fake<IPageManager<TechnologyPage>>();
            _classUnderTest = new MammaMRTImportReportService(fakeLogger, _kafkaProducerFake, _moduleContextFake, _dicomValueParserFake, _httpAuthContextFake,
                _patientIndicationPageManagerFake, _technologyPageManagerFake, _reportManagerFake, _reportPageServiceFake);
        }
        ...
        public async void ImportReport_ValuesImportable_ValuesImportedProperly()
        {
            ...
            var expectedPatientIndicationPageId = Guid.NewGuid();
            ...
            {
                new PageInfo { TypeName = PageTypeNames.MammaMrtPatientIndication },
                new PageInfo { TypeName = PageTypeNames.MammaMrtTechnology }
            };
            ...
            A.CallTo(() => _moduleContextFake.LocalizationPageName).Returns(PageTypeNames.MammaMrtPatientIndication);
            A.CallTo(() => _moduleContextFake.TechnologyPageName).Returns(PageTypeNames.MammaMrtTechnology);
            A.CallTo(() => _moduleContextFake.TotalPageCount).Returns((byte) 2);
            A.CallTo(() => _moduleContextFake.Type).Returns(ModuleType.MammaMRT);
            ...
            A.CallTo(() => _dicomValueParserFake.ParseAgeString(A<StringImportValue>.That.Matches(v => v.RadioReportId == "uni_01_009")))
                .Returns(expectedAge);
            A.CallTo(() => _patientIndicationPageManagerFake.AddAsync(A<PatientIndicationPage>.Ignored, A<bool>.Ignored)).Returns(new PatientIndicationPage
            {
                Id = expectedPatientIndicationPageId
            });
            ...
            Assert.Equal(ModuleType.MammaMRT.GetReportTypeName(), result.ExaminationType);
            ...
            Assert.Contains(result.Pages, p => p.PageId == expectedPatientIndicationPageId);
