...
using RadioReport.CardioMRT.Domain.Services;
...
namespace RadioReport.Tests.Modules.CardioMRT
{
    public class CardioMrtImportReportServiceTests
    {
        private readonly CardioMRTImportReportService _classUnderTest;
        ...
        private readonly IPageManager<AreaOfExaminationPage> _areaOfExaminationPageManagerFake;
        private readonly IPageManager<TechnologyPage> _technologyPageManagerFake;

        public CardioMrtImportReportServiceTests()
        {
            var fakeLogger = A.Fake<ILogger<CardioMRTImportReportService>>();
            ...
            _areaOfExaminationPageManagerFake = A.Fake<IPageManager<AreaOfExaminationPage>>();
            _technologyPageManagerFake = A.Fake<IPageManager<TechnologyPage>>();
            _classUnderTest = new CardioMRTImportReportService(fakeLogger, _kafkaProducerFake, _moduleContextFake, _dicomValueParserFake, _httpAuthContextFake,
                _areaOfExaminationPageManagerFake, _technologyPageManagerFake, _reportManagerFake, _reportPageServiceFake);
        }
        ...
        public async void ImportReport_ValuesImportable_ValuesImportedProperly()
        {
            ...
            var expectedAreaOfExaminationPageId = Guid.NewGuid();
            ...
            {
                new PageInfo { TypeName = PageTypeNames.CardioMrtAreaOfExamination },
                new PageInfo { TypeName = PageTypeNames.CardioMrtTechnology }
            };

            A.CallTo(() => _kafkaProducerFake.SendSynchronousRequest<GetOrAddPatientSyncResponse>(
                A<GetOrAddPatientSyncRequest>.Ignored,
                A<HttpAuthContext>.Ignored, A<int>.Ignored, A<CancellationToken>.Ignored)).Returns(addPatientResponse);
            A.CallTo(() => _moduleContextFake.LocalizationPageName).Returns(PageTypeNames.CardioMrtAreaOfExamination);
            A.CallTo(() => _moduleContextFake.TechnologyPageName).Returns(PageTypeNames.CardioMrtTechnology);
            A.CallTo(() => _moduleContextFake.TotalPageCount).Returns((byte) 2);
            A.CallTo(() => _moduleContextFake.Type).Returns(ModuleType.CardioMRT);
            ...
            A.CallTo(() =>
                    _dicomValueParserFake.ParseDate(
                        A<StringImportValue>.That.Matches(v => v.RadioReportId == "rep_x_000002")))
                .Returns(expectedExaminationDate);
            A.CallTo(() =>
                    _dicomValueParserFake.ParseAgeString(
                        A<StringImportValue>.That.Matches(v => v.RadioReportId == "uni_01_009")))
                .Returns(expectedAge);
            A.CallTo(() =>
                _areaOfExaminationPageManagerFake.AddAsync(A<AreaOfExaminationPage>.Ignored, A<bool>.Ignored)).Returns(
                new AreaOfExaminationPage
                {
                    Id = expectedAreaOfExaminationPageId
                });
            A.CallTo(() => _technologyPageManagerFake.AddAsync(A<TechnologyPage>.Ignored, A<bool>.Ignored)).Returns(
                new TechnologyPage
                {
            ...
            Assert.Equal(ModuleType.CardioMRT.GetReportTypeName(), result.ExaminationType);
            ...
            Assert.Contains(result.Pages, p => p.PageId == expectedAreaOfExaminationPageId);
