import { Injectable } from '@angular/core';

import { IMapper } from '@interfaces/mapper.interface';
import { SequenceApiModel } from '@models/hipMRT/api/technology/sequence-api.model';
import { SequenceUiModel } from '@models/hipMRT/ui/technology/sequence-ui.model';

@Injectable({
  providedIn: 'root'
})
export class SequenceMapper implements IMapper<SequenceApiModel, SequenceUiModel> {
  constructor() {
    automapper.createMap('Sequence_HipMRT_ApiModel', 'Sequence_HipMRT_UiModel').convertToType(SequenceUiModel);
    automapper.createMap('Sequence_HipMRT_UiModel', 'Sequence_HipMRT_ApiModel').convertToType(SequenceApiModel);
  }

  mapToAPI(source: SequenceUiModel, out: SequenceApiModel): SequenceApiModel {
    out = automapper.map('Sequence_HipMRT_UiModel', 'Sequence_HipMRT_ApiModel', source);
    return out;
  }

  mapToUI(source: SequenceApiModel, out: SequenceUiModel): SequenceUiModel {
    out = automapper.map('Sequence_HipMRT_ApiModel', 'Sequence_HipMRT_UiModel', source);
    return out;
  }
}
