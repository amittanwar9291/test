...
@Component({
  selector: 'rr-ventricle-differential-diagnosis-tab',
  templateUrl: './ventricle-differential-diagnosis-tab.component.html',
  styleUrls: ['./ventricle-differential-diagnosis-tab.component.scss'],
...
export class DifferentialDiagnosisTabComponent implements OnInit, OnDestroy {
  ...
  diagnosisTypeEnum = CerebrospinalVentricleDiagnosisTypeEnum;

  subscriptions: Subscription[] = [];
  ddlW041: DropdownModel[];
  ...
  private initTranslations() {
    this.subscriptions.push(
      this.translateService.stream('headMRT.cerebrospinalFluid.w041.options').subscribe((out: DropdownModel[]) => {
        this.ddlW041 = Object.keys(out).map(e => out[e]);
      })
    );
  }
